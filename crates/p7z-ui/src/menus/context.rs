...
use crate::*;
use p7z_commands::Action as ExplorerAction;
            ...
            let _ = view.update(cx, |this, cx| {
                ...
                match &action {
                    ...
                    Action::CopyPaths => {
                        ...
                        this.notify_message(tr("paths-copied").into(), cx);
                    }
...
pub(crate) fn build(
                ...
                !browser.selected.is_empty()
                    && browser.selected.iter().all(|path| {
                        browser.rows.iter().any(|entry| {
                            &entry.path == path
                                && !entry.directory
                                && p7z_commands::may_extract(Path::new(path))
                        })
                    }),
            )
        })
        ...
        .flatten()
    {
        ...
        if !busy && !paths.is_empty() {
            ...
            let name = p7z_commands::archive_name(&sources, folder.is_some(), false);
            menu = menu.separator();
            for format in [
                None,
                Some(p7z_commands::ArchiveFormat::Zip),
                Some(p7z_commands::ArchiveFormat::SevenZip),
            ] {
...
fn archive_file_menu(
    ...
    if paths.len() == 1 {
        ...
        menu = menu
            .submenu(tr("archive-open-type"), {
                let mut menu = Menu::new();
                for action in p7z_commands::ACTIONS.iter().copied() {
    ...
    let folder_label = if paths.len() == 1 {
        tf(
            ...
            &[(
                "name",
                p7z_commands::extract_folder(Path::new(&paths[0])).into(),
            )],
        )
    } else {
