...
use zip_engine::{Cancellation, Catalog, CreateOptions, Edit, Engine, Overwrite, Progress};
...
pub enum Request {
    ...
    Open(PathBuf),
    OpenAs(PathBuf, zip_commands::OpenType),
    QuickCompress {
        paths: Vec<PathBuf>,
        format: zip_commands::ArchiveFormat,
        email: bool,
    },
    Extract {
        catalog: Catalog,
...
impl Request {
    ...
    pub fn run(
        ...
        match result {
            Err(error)
                if self.prompts_for_password() && zip_engine::needs_password(&error) =>
            {
...
fn resolve_address(engine: &Engine, path: PathBuf, cancel: &Cancellation) -> Result<Outcome> {
    ...
    match engine.list(&archive, "", cancel) {
        ...
        Err(error) if zip_engine::needs_password(&error) => {
...
fn checksum_report(output: zip_engine::Output) -> Outcome {
    Outcome::Report(
