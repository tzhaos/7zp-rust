...
pub use zip_commands::Action as ExplorerAction;
    ...
    if path.parent() != Some(std::env::temp_dir().as_path())
        || !path.file_name().is_some_and(|name| {
            name.to_string_lossy()
                .starts_with(zip_commands::REQUEST_PREFIX)
        })
    {
        anyhow::bail!(zip_core::i18n::tr("shell-request-invalid"));
    }
    let bytes = std::fs::read(path)
        .with_context(|| format!("Cannot read Explorer request {}", path.display()))?;
    let request = serde_json::from_slice::<zip_commands::Request>(&bytes)
...
pub fn parse_action(value: &str) -> anyhow::Result<ExplorerAction> {
    zip_commands::ACTIONS
        .iter()
        .copied()
        .find(|action| action.argument() == value)
        .ok_or_else(|| {
            anyhow::anyhow!(zip_core::i18n::tf(
