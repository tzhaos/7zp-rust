use super::{
    Cancellation, Catalog, CreateOptions, Entry, Format, Overwrite, Progress, Volume, progress,
};
use anyhow::{Context, Result, bail};
use zip_commands::OpenType;
...
impl Engine {
    ...
    fn command(&self) -> Command {
        ...
        #[cfg(windows)]
        {
            ...
            let priority = if zip_core::settings::LOW_PRIORITY.load(Ordering::Relaxed) {
                0x00004000
            } else {
                0
            };
    ...
    pub(super) fn run_in(
        ...
        let text = String::from_utf8(stdout).map_err(|error| {
            anyhow::anyhow!(tf(
                "engine-encoding-invalid",
                &[
                    ("code", format!("{:?}", status.code()).into()),
                    ("error", error.utf8_error().to_string().into()),
                    ("output", error.as_bytes().escape_ascii().to_string().into()),
                ],
            ))
        })?;
        match status.code() {
    ...
    pub fn create(
        ...
        if replacing && options.volume != Volume::None {
            bail!(tr("split-update-unsupported"));
        }
        if format.single_file() && (sources.len() != 1 || sources[0].is_dir()) {
        ...
        let filename = destination
            ...
            .to_string_lossy();
        let filename = if options.volume != Volume::None {
        ...
        let mut args = vec![
            "a".into(),
            format!("-t{}", format.value()).into(),
            format!("-mx={}", options.level.argument()).into(),
            "-sccUTF-8".into(),
            "-spd".into(),
        ];
        if !password.is_empty() {
            ...
            if format.supports_header_encryption() && options.encrypt_names {
        ...
        if format == Format::SevenZip {
            args.push(format!("-m0={}", options.method.argument()).into());
            args.push(format!("-ms={}", if options.solid { "on" } else { "off" }).into());
        } else if format == Format::Zip {
            args.push(format!("-mm={}", options.method.argument()).into());
        }
        if let Some(threads) = options.threads.argument() {
            args.push(format!("-mmt={threads}").into());
        }
        if let Some(volume) = options.volume.argument() {
            args.push(format!("-v{volume}").into());
        }
...
pub(super) fn input_list(sources: &[PathBuf]) -> Result<tempfile::TempPath> {
    let mut file = tempfile::NamedTempFile::new()?;
    for path in sources {
        writeln!(file, "{}", path.display())?;
    }
    file.flush()?;
    // 7-Zip opens listfiles without sharing write access on Windows.
