use super::*;
use sevenzip_platform::ShellAction;

impl Workspace {
    pub fn attach_system(
        &mut self,
        receiver: std::sync::mpsc::Receiver<sevenzip_platform::Command>,
        args: Vec<String>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn dispatch(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let action = match sevenzip_platform::parse_action(argument) {
            Ok(action) => action,
            Err(error) => {
        ...
        match action {
            ...
            ShellAction::Checksum(method) => self.execute(
                Request::Checksum {
                    paths: paths.iter().map(PathBuf::from).collect(),
                    kind: sevenzip_application::ChecksumKind::Hash(method),
                },
                String::new(),
                cx,
            ),
            ShellAction::GenerateChecksum => self.execute(
                Request::Checksum {
                    paths: paths.iter().map(PathBuf::from).collect(),
                    kind: sevenzip_application::ChecksumKind::Generate,
                },
                String::new(),
                cx,
            ),
            ShellAction::VerifyChecksum => self.execute(
                Request::Checksum {
                    paths: paths.iter().map(PathBuf::from).collect(),
                    kind: sevenzip_application::ChecksumKind::Verify,
                },
                String::new(),
                cx,
            ),
