use super::*;
pub(super) use sevenzip_application::filesystem::Directory;

pub(super) use sevenzip_application::browser::Location;
        ...
        if self.browser.view().directory.is_some() {
            if entry.directory {
                ...
                self.start(tr("opening"), cx, move |_| {
                    sevenzip_platform::open_file(std::path::Path::new(&path))?;
