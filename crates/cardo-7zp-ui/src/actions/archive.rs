...
impl Workspace {
    ...
    pub(crate) fn compress_from_context(
        ...
        directory: PathBuf,
        name: String,
        format: Option<cardo_7zp_commands::ArchiveFormat>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(crate) fn reveal_completion(&mut self, path: PathBuf, cx: &mut Context<Self>) {
        match cardo_7zp_platform::open_directory(&path) {
            Ok(true) => {}
    ...
    pub(crate) fn begin_extract(
        ...
        self.execute(
            Request::PrepareExtraction(cardo_7zp_requests::extraction::Extraction {
                catalog,
                selected,
                parent,
                folder,
