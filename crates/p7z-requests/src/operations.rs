...
use p7z_engine::{Cancellation, Catalog, CreateOptions, Edit, Engine, Overwrite, Progress};
...
#[derive(Clone)]
pub enum Request {
    PrepareExtraction(super::extraction::Extraction),
    ...
    Open(PathBuf),
    OpenAs(PathBuf, p7z_commands::OpenType),
    QuickCompress {
        paths: Vec<PathBuf>,
        format: p7z_commands::ArchiveFormat,
        email: bool,
    },
    Extract {
        catalog: Catalog,
...
pub enum ChecksumKind {
    Hash(p7z_commands::HashMethod),
    Generate,
    Verify,
}

pub enum Outcome {
    ExtractionReady(super::extraction::ExtractionPlan),
    Comment(String),
...
impl Request {
    pub fn label(&self) -> &'static str {
        tr(match self {
            Self::PrepareExtraction(_) => "extract-preparing",
    ...
    pub fn extraction_paths(&self) -> Option<(PathBuf, Option<PathBuf>)> {
        match self {
            Self::PrepareExtraction(extraction) => Some((
                extraction.catalog.path.clone(),
                Some(extraction.parent.join(&extraction.folder)),
            )),
            Self::Extract {
                catalog,
                parent,
    ...
    pub fn run(
        ...
        let result = match &self {
            Self::PrepareExtraction(extraction) => extraction
                .clone()
                .prepare(cancel)
                .map(Outcome::ExtractionReady),
        ...
        match result {
            Err(error)
                if self.prompts_for_password() && p7z_engine::needs_password(&error) =>
            {
    ...
    fn perform(
        ...
        match self {
            ...
            Self::Extract {
                ...
                if !super::extraction::valid_folder(folder) {
...
fn resolve_address(engine: &Engine, path: PathBuf, cancel: &Cancellation) -> Result<Outcome> {
    ...
    match engine.list(&archive, "", cancel) {
        ...
        Err(error) if p7z_engine::needs_password(&error) => {
...
fn checksum_report(output: p7z_engine::Output) -> Outcome {
    Outcome::Report(
