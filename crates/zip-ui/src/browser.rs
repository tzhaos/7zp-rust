use super::*;
pub(super) use zip_requests::filesystem::Directory;

pub(super) use zip_requests::browser::Location;
...
impl Workspace {
    ...
    pub(super) fn can_open_inside(&self) -> bool {
        ...
        if self.browser.view().directory.is_some() {
            entry.directory || zip_commands::may_extract(Path::new(&entry.path))
        } else {
