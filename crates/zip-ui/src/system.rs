use super::*;
use zip_platform::ExplorerAction;

impl Workspace {
    pub fn attach_system(
        &mut self,
        receiver: std::sync::mpsc::Receiver<zip_platform::Command>,
        args: Vec<String>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn dispatch(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if argument == "--shell-request" {
            ...
            let read = cx
                .background_executor()
                .spawn(async move { zip_platform::read_shell_request(&path) });
        ...
        let action = match zip_platform::parse_action(argument) {
            Ok(action) => action,
            Err(error) => {
        ...
        match action {
            ...
            ExplorerAction::Checksum(method) => self.execute(
                Request::Checksum {
                    paths: paths.iter().map(PathBuf::from).collect(),
                    kind: zip_requests::ChecksumKind::Hash(method),
                },
                String::new(),
                cx,
            ),
            ExplorerAction::GenerateChecksum => self.execute(
                Request::Checksum {
                    paths: paths.iter().map(PathBuf::from).collect(),
                    kind: zip_requests::ChecksumKind::Generate,
                },
                String::new(),
                cx,
            ),
            ExplorerAction::VerifyChecksum => self.execute(
                Request::Checksum {
                    paths: paths.iter().map(PathBuf::from).collect(),
                    kind: zip_requests::ChecksumKind::Verify,
                },
                String::new(),
                cx,
            ),
