...
use crate::*;
use zip_core::settings::Preferences;

impl Workspace {
    pub(crate) fn open_default_apps(&self, cx: &mut App) {
        cx.open_url(zip_platform::DEFAULT_APPS_URI);
    }
    ...
    pub(crate) fn change_language(
        &mut self,
        language: zip_core::i18n::Language,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        ...
        let save = cx
            .background_executor()
            .spawn(async move { zip_core::i18n::save(language) });
...
impl Workspace {
    ...
    pub(crate) fn apply_preferences(&mut self, value: Preferences, cx: &mut Context<Self>) {
        zip_core::settings::LOW_PRIORITY.store(value.low_priority, Ordering::Relaxed);
    ...
    pub(crate) fn check_updates_quietly(&mut self, cx: &mut Context<Self>) {
        self.update_status = Some(zip_requests::update::Status::Checking);
        let job = cx
            .background_executor()
            .spawn(async { zip_requests::update::check() });
        self.update_task = Some(cx.spawn(async move |view, cx| {
            ...
            let _ = view.update(cx, |this, cx| {
                if let zip_requests::update::Status::Available { version, .. } = &status {
                    this.notify_message(tf(
                        "update-available",
