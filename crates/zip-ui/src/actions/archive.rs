...
impl Workspace {
    ...
    pub(crate) fn reveal_completion(&mut self, path: PathBuf, cx: &mut Context<Self>) {
        match zip_platform::open_directory(&path) {
            Ok(true) => {}
    ...
    pub(crate) fn extract_dialog(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let name = zip_commands::extract_folder(&c.path);
    ...
    pub(crate) fn quick_extract(&mut self, selected: bool, cx: &mut Context<Self>) {
        ...
        let folder = zip_commands::extract_folder(&catalog.path);
    ...
    pub(crate) fn begin_extract(
        ...
        self.execute(
            Request::PrepareExtraction(zip_requests::extraction::Extraction {
                catalog,
                selected,
                parent,
                folder,
    ...
    pub(crate) fn extract(
        ...
        let save = cx
            .background_executor()
            .spawn(async move { zip_core::settings::save_destination(&saved_parent) });
        cx.spawn(async move |view, cx| {
    ...
    pub(crate) fn save_copy(&mut self, cx: &mut Context<Self>) {
        ...
        self.start(tr("saving"), cx, move |_| {
            ...
            zip_requests::filesystem::copy_file(&c.path, &destination)?;
    ...
    pub(crate) fn drop_files(
        ...
        let archive = paths.len() == 1 && zip_commands::opens_directly(&paths[0]);
        if archive {
