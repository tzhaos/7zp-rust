...
impl PreferencesForm {
    pub(super) fn step_font_size(
        ...
        let next = zip_core::settings::Appearance::clamp_font_size(self.font_size + delta);
    ...
    pub(crate) fn save(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.error = None;
        let appearance = zip_core::settings::Appearance {
            font_family,
        ...
        let job = cx.background_executor().spawn(async move {
            zip_core::settings::prepare_temp_directory(&value.temp_directory)?;
            if registration_changed {
                zip_platform::configure(&std::env::current_exe()?, &value)?;
            }
            zip_core::settings::save_preferences(&value)?;
            zip_core::settings::save_appearance(&appearance)?;
                        ...
                        App::defer(cx, move |cx| {
                            let _ = handle.update(cx, |_, window, cx| {
                                let _ = owner.update(cx, |owner, cx| {
                                    ...
                                    if language != zip_core::i18n::current() {
