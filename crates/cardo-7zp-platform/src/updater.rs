...
pub fn job_directory() -> Result<tempfile::TempDir> {
    let root = cardo_7zp_core::settings::directory()?.join("updates");
...
impl Prepared {
    pub fn launch(self, cancel: &AtomicBool) -> Result<()> {
        ...
        let store = cardo_7zp_core::settings::store()?;
        store.write_json("update-pending.json", &Some(&self.plan))?;
        ...
        let mut child = match spawn {
            Ok(child) => child,
            Err(error) => {
                store.write_json("update-pending.json", &Option::<Plan>::None)?;
...
fn backup(plan: &mut Plan) -> Result<()> {
    ...
    plan.applying = true;
    cardo_7zp_core::settings::store()?.write_json("update-pending.json", &Some(&plan))?;
    Ok(())
}
...
pub fn apply() -> Result<()> {
    let store = cardo_7zp_core::settings::store()?;
    let mut plan = store
        .read_json::<Option<Plan>>("update-pending.json")?
        .flatten()
        .context(tr("update-package-invalid"))?;
    validate_plan(&plan)?;
...
pub fn recover() -> Result<Option<String>> {
    ...
    if let Some(outcome) = store
        .read_json::<Option<Outcome>>("update-result.json")?
        .flatten()
    {
        let root = cardo_7zp_core::settings::directory()?.join("updates");
        ...
        store.write_json("update-result.json", &Option::<Outcome>::None)?;
