...
impl Workspace {
    pub(in crate::ui) fn settings_page(&self, cx: &Context<Self>) -> Option<Div> {
    ...
    pub(in crate::ui) fn subpage_header(&self, title: &'static str, cx: &Context<Self>) -> Div {
        navigation_row(cx)
            .child(
                icon_button("page-back", "ArrowLeft", tr("back"), cx)
                    .disabled(self.busy || self.settings_busy(cx))
                    .on_click(
