...
pub(in crate::ui) struct ErrorDialog {
    summary: String,
    path: Option<PathBuf>,
    details: String,
    expanded: bool,
...
impl Workspace {
    pub(in crate::ui) fn show_task_error(
        &mut self,
        error: anyhow::Error,
        extracting: bool,
        cx: &mut Context<Self>,
        ...
        let mut path = None;
        if let Some(source) = error.downcast_ref::<crate::archive::SourceError>() {
        ...
        self.modal = Some(Modal::Error(ErrorDialog {
        ...
        v_flex()
            ...
            .child(
                ...
                .on_click(cx.listener(|this, _, _, cx| {
                    if let Some(Modal::Error(error)) = &mut this.modal {
