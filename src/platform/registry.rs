...
use anyhow::Result;
use sevenzip_shell_api::{ACTIONS, CLSID, EXTENSIONS, SHELL_KEY};
use std::path::Path;
...
pub fn register(executable: &Path, dll: &Path) -> Result<()> {
    let preferences = crate::settings::load_preferences()?;
...
pub fn configure(executable: &Path, preferences: &crate::settings::Preferences) -> Result<()> {
    let user = RegKey::predef(HKEY_CURRENT_USER);
    let classes = user.create_subkey(r"Software\Classes")?.0;
    let exe = executable.display().to_string();
